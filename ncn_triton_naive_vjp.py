...
def backward_kernel(
    ...
    eye = tl.where(tl.arange(0, EMBED_DIM)[:, None] == tl.arange(0, EMBED_DIM)[None, :], 1.0, 0.0).to(tl.float32)[None].broadcast_to((GROUP, EMBED_DIM, EMBED_DIM)) 
    ...
    for start_j in range(hi-1, lo-1, -1):
        ...
        dyi_dT = (grad_norm2 * grad_activation * grad_norm1 )[:,:,None] * eye 
        dya_dT = (grad_activation * grad_norm1 )[:,:,None] * eye 

        dT_dWi = xj[:, :, None] * xi[:, None, :] * (1.0-ALPHA)  # (B,E,1) * (B,1,E) -> (B,E,E) 
        dT_dWj = xj[:, :, None] * xj[:, None, :] * (1.0-ALPHA)  # (B,E,1) * (B,1,E) -> (B,E,E) 

        dyi_dWi = tl.dot(dyi_dT, dT_dWi)
        dyi_dWj = tl.dot(dyi_dT, dT_dWj)

        dya_dWi = tl.dot(dya_dT, dT_dWi)
        dya_dWj = tl.dot(dya_dT, dT_dWj)

        # vjp
        dWi_group = tl.sum(dyi[:, :, None] * dyi_dWi, axis=1) + tl.sum(dya[:, :, None] * dya_dWi, axis=1) # sum((E,1) * (E,E),0) -> (E)
        dWj_group = tl.sum(dyi[:, :, None] * dyi_dWj, axis=1) + tl.sum(dya[:, :, None] * dya_dWj, axis=1) # sum((E,1) * (E,E),0) -> (E)
        ...
        dT_dxi = ALPHA * eye + (1.0-ALPHA) * (xj[:, :, None] * Wi[None, None, :])   # (E,E) + ( (E,1)*(1,E) ) = (E,E)

        # i==j
        xj_mask = tl.arange(0, GROUP) == start_j
        #import pdb; pdb.set_trace()

        dT_dxij = ALPHA * eye + (1.0-ALPHA) * (xj[:, :, None] * (Wi + Wj)[None, None, :] + sim[:, None, None] * eye)
        dT_dxi = tl.where(xj_mask[:, None, None], dT_dxij, dT_dxi)

        dz_dxa = eye
        dyi_dxi = eye + tl.dot(dyi_dT, dT_dxi)
        dya_dxi = tl.dot(dya_dT, dT_dxi)
        dyi_dxa = grad_norm2[:,:,None] * dz_dxa
        dya_dxa = eye # tl.broadcast_to(eye[None], (GROUP, EMBED_DIM, EMBED_DIM))

        # vjp
        dxi = tl.sum(dyi[:, :, None] * dyi_dxi, axis=1) + tl.sum(dya[:, :, None] * dya_dxi, axis=1)  # (E,)
        dxa = tl.sum(dyi[:, :, None] * dyi_dxa, axis=1) + tl.sum(dya[:, :, None] * dya_dxa, axis=1)  # (E,)

        #if offs_group == 0 and offs_batch == 14:
        #    import pdb; pdb.set_trace()

        # i!=j
        dT_dxj = (1.0-ALPHA) * (xj[:, :, None] * Wj[None, None, :] + sim[:, None, None] * eye)
        dyi_dxj = tl.dot(dyi_dT, dT_dxj)
        dya_dxj = tl.dot(dya_dT, dT_dxj)


        dxj_group = tl.sum(dyi[:, :, None] * dyi_dxj, axis=1) + tl.sum(dya[:, :, None] * dya_dxj, axis=1)  # (B,E)
        dxj = tl.sum(tl.where((xj_mask==False)[:, None], dxj_group, 0.0), axis=0)  # (B,E) -> (E,)
    ...
    #if offs_batch==0 and offs_group==0:
    #    import pdb; pdb.set_trace()
...
class NCN(torch.autograd.Function):
...
def torch_reference(x, W, norm_alpha, norm_beta, norm_gamma, group_tensor, seqlen_offsets, ALPHA, ACTIVATION, GROUP):
    xa_init = torch.zeros_like(x)
    B, L, E = x.shape
    
    norm_alpha_1, norm_alpha_2 = norm_alpha[0], norm_alpha[1]
    norm_gamma_1, norm_gamma_2 = norm_gamma[0:E], norm_gamma[E:2*E]
    norm_beta_1, norm_beta_2 = norm_beta[0:E], norm_beta[E:2*E]
    
    yi_outputs, ya_outputs = [], []

    for i in range(0, L, GROUP):
        xi = x[:, i:i+GROUP]
        xa = xa_init[:, i:i+GROUP]

        for j in range(GROUP):
            
            #import pdb; pdb.set_trace()
            xj = xi[:, j].unsqueeze(1).broadcast_to(xi.shape)
            #xj = xi[:, j].detach().clone().unsqueeze(1).broadcast_to(xi.shape)
            #xj.requires_grad = False
            
            #import pdb; pdb.set_trace()

            phi = torch.cat((xi, xj), dim=-1)

            T = ALPHA * xi + (1.0-ALPHA) * torch.matmul(phi, W)[:, :, None] * xj  # (B,2E)
            T_norm = norm_gamma_1 * ref_tanh(norm_alpha_1 * T) + norm_beta_1

            if ACTIVATION=="leaky-relu":
                F = ref_leaky_relu(T_norm)
            else:
                F = T_norm
            
            z = xa + F
            z_norm = norm_gamma_2 * ref_tanh(norm_alpha_2 * z) + norm_beta_2
        ...
        yi_outputs.append(xi)
        ya_outputs.append(xa)
        #return yi, ya



    return torch.cat(yi_outputs, 1), torch.cat(ya_outputs, 1)




def test_op(BATCH_SIZE, SEQ_LEN, EMBED_DIM, dtype=torch.float32):
    x = (
        torch.ones(
    ...
    W = (
        torch.ones(
            (EMBED_DIM*2), dtype=dtype, device="cuda"
        )
        .normal_(mean=0.0, std=1/(2.0 * EMBED_DIM))
        .requires_grad_()
    )

    norm_alpha = (torch.ones((2,), dtype=dtype, device="cuda").mul_(0.5).requires_grad_())
    norm_beta = (torch.zeros((2*EMBED_DIM,), dtype=dtype, device="cuda").add_(-0.1).requires_grad_())
    norm_gamma = (torch.ones((2*EMBED_DIM,), dtype=dtype, device="cuda").mul_(0.5).requires_grad_())
    ...
    ref_yi, ref_ya = torch_reference(x, W, norm_alpha, norm_beta, norm_gamma, group_tensor, seqlen_offsets, ALPHA, ACTIVATION, GROUP)
    ref_yi.backward(dyi)
    #print("torch grad", x.grad[5, 0])
    ref_dW, W.grad = W.grad.clone(), None
    ref_dx, x.grad = x.grad.clone(), None
    ref_dnorm_alpha, norm_alpha.grad = norm_alpha.grad.clone(), None
    ref_dnorm_gamma, norm_gamma.grad = norm_gamma.grad.clone(), None
    ref_dnorm_beta, norm_beta.grad = norm_beta.grad.clone(), None

    tri_yi, tri_ya = NCN.apply(x, W, norm_alpha, norm_beta, norm_gamma, group_tensor, seqlen_offsets, ALPHA, ACTIVATION)
    ...
    tri_dnorm_alpha, norm_alpha.grad = norm_alpha.grad.clone(), None
    tri_dnorm_gamma, norm_gamma.grad = norm_gamma.grad.clone(), None
    tri_dnorm_beta, norm_beta.grad = norm_beta.grad.clone(), None
    ...
    rtol = 0.0
    atol = 1e-2
    print(triton.testing.assert_close(tri_yi, ref_yi, atol=1e-2))
    print(triton.testing.assert_close(tri_dW, ref_dW, atol=1e-2))
    print(triton.testing.assert_close(tri_dx, ref_dx, atol=1e-2))
    print(triton.testing.assert_close(tri_dnorm_alpha, ref_dnorm_alpha, atol=1e-2))
    print(triton.testing.assert_close(tri_dnorm_beta, ref_dnorm_beta, atol=1e-2))
    print(triton.testing.assert_close(tri_dnorm_gamma, ref_dnorm_gamma, atol=1e-2))


    print("tri", tri_dx, "torch", ref_dx)
    print("Allclose: yi", torch.allclose(ref_yi, tri_yi, atol=atol, rtol=rtol))
    print("Allclose: ya", torch.allclose(ref_ya, tri_ya, atol=atol, rtol=rtol))
    print("Allclose: dW", torch.allclose(ref_dW, tri_dW, atol=atol, rtol=rtol))
    print("Allclose: dX", torch.allclose(ref_dx, tri_dx, atol=atol, rtol=rtol))
...
if __name__ == "__main__":
    test_op(BATCH_SIZE=32, SEQ_LEN=256, EMBED_DIM=16)
    #test_op(BATCH_SIZE=16, SEQ_LEN=4096, EMBED_DIM=64, causal=False)
    #test_op(BATCH_SIZE=16, SEQ_LEN=4096, EMBED_DIM=64, causal=False)
    #test_op(BATCH_SIZE=16, SEQ_LEN=4096, EMBED_DIM=64, causal=False)
    print("PASSED")